...
pub fn spawn_loot(rand: &mut StdbRand, mut pos: DVec3, stack: ItemStack, spread: f32) {
    if spread != 0.0 {
        pos += rand.rand.next_float_vec()
            .mul(spread)
            .as_dvec3()
    ...
    let entity = Item::new_with(|base, item| {
        ...
        base.pos = pos;
        base.vel.x = rand.rand.next_double() * 0.2 - 0.1;
        base.vel.y = 0.2;
        base.vel.z = rand.rand.next_double() * 0.2 - 0.1;
        item.stack = stack;
        item.frozen_time = 10;
    });

    spawn_entity_inner(entity);
}
...
pub fn spawn_block_loot(rand: &mut StdbRand, pos: IVec3, id: u8, metadata: u8, chance: f32) {
    let tries = get_block_loot_tries(rand, id, metadata);
    for try_num in 0..tries {
        if rand.next_float() <= get_block_loot_chance(id, metadata, try_num, chance) {
            let stack = get_block_loot_stack(rand, id, metadata, try_num);
            if !stack.is_empty() {
                spawn_loot(rand, pos.as_dvec3() + 0.5, stack, 0.7);
            }
        }
    }
}
...
fn get_block_loot_tries(rand: &mut StdbRand, id: u8, _metadata: u8) -> u8 {
    match id {
        block::AIR => 0,
        block::BOOKSHELF => 0,
        block::CAKE => 0,
        ...
        block::LAVA_STILL => 0,
        block::GLASS => 0,
        block::GLOWSTONE => 2 + rand.rand.next_int_bounded(3) as u8,
        block::ICE => 0,
        block::LEAVES if rand.rand.next_int_bounded(20) != 0 => 0,
        block::SPAWNER => 0,
        block::LAPIS_ORE => 4 + rand.rand.next_int_bounded(5) as u8,
        block::PISTON_EXT |
        block::PISTON_MOVING => 0,
        block::PORTAL => 0,
        block::REDSTONE_ORE |
        block::REDSTONE_ORE_LIT => 4 + rand.rand.next_int_bounded(2) as u8,
        block::SNOW => 0,
        block::SNOW_BLOCK => 4,
        block::DOUBLE_SLAB => 2,
        block::TNT => 0,
...
fn get_block_loot_chance(id: u8, metadata: u8, try_num: u8, default_chance: f32) -> f32 {
    match id {
...
fn get_block_loot_stack(rand: &mut StdbRand, id: u8, metadata: u8, try_num: u8) -> ItemStack {
    match id {
        ...
        // Gravel.
        block::GRAVEL if rand.rand.next_int_bounded(10) == 0 => ItemStack::new_single(item::FLINT, 0),
        // Leaves.
        ...
        // Tall grass.
        block::TALL_GRASS if rand.rand.next_int_bounded(8) == 0 => ItemStack::new_single(item::WHEAT_SEEDS, 0),
